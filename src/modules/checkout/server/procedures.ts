...
export const checkoutRouter = createTRPCRouter({
  verify: protectedProcedure.mutation(async ({ ctx }) => {
    const user = await ctx.payload.findByID({
      ...
      depth: 0, // user.tenants[0].tenant = string (tenantId)
    })
  ...
  purchase: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      //  TODO: Thow error if stripe error not submitted
        ...
        products.docs.map(product => ({
          ...
          price_data: {
            ...
            product_data: {
              name: product.name,
              metadata: {
                stripeAccId: tenant.stripeAccountId,
                id: product.id,
                name: product.name,
                price: product.price,
              } as ProductMetadata,
  ...
  getProducts: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        ...data,
        totalPrice: data.docs.reduce((acc, curr) => acc + curr.price, 0),
